...
class Client {
  ...
  /**
   * Search for a FHIR resource, within a compartment, or the entire system
   *
   * @example
   *
   * // Using promises
   ...
   *
   * @return {String} level - The search level
   */

  search({ resourceType, compartment, searchParams } = {}) {
    let level;

    if (compartment && resourceType && searchParams) {
      this.compartmentSearch({ resourceType, compartment, searchParams });
      level = 'compartment';
    } else if (resourceType && searchParams) {
      this.resourceSearch({ resourceType, searchParams });
      level = 'resource';
    } else if (searchParams) {
      this.systemSearch({ searchParams });
      level = 'system';
    }
  ...
  compartmentSearch({ resourceType, compartment, searchParams } = {}) {
    const compartmentType = compartment.resourceType;
    const compartmentId = compartment.id;
    const query = new URLSearchParams(searchParams);
    const relativePath = `/${compartmentType}/${compartmentId}/${resourceType}?${query}`;
   ...
   * @param {string} params.id - The FHIR id for the resource.
   *
   * @return {String} level - The history level
   */
  ...
  history({ resourceType, id } = {}) {
    ...
    if (id && resourceType) {
      this.resourceHistory({ resourceType, id });
      level = 'resource';
    } else if (resourceType) {
      this.typeHistory({ resourceType });
      level = 'type';
    } else {
      this.systemHistory();
      level = 'system';
