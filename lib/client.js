...
class Client {
   ...
   * };
   *
   * client.operation({resourceType: 'ConceptMap', name: 'tranlate', method: 'get', input}).
   *   then(result => console.log(result)).
   *   catch(e => console.error(e));
   *
   ...
   * @param {Object} [params.input] - Optional options object
   * @param {Object} [params.options] - Optional options object
   * @param {Object} [params.options.headers] - Optional headers to add to the
   *   request
   *
  ...
  operation({ name, resourceType, id, method='post', input = {}, options = {} } = {}) {
    ...
    if (method === 'post') {
      return this.httpClient.post(url.join(''), options, body);
    }
    if (method === 'get') {
      if (input) {
