...
class Client {
   ...
   *   each request
   * @param {String} config.key TLS key for FHIR server
   * @param {String} config.cert TLS certificate for FHIR server
   * @throws An error will be thrown unless baseUrl is a non-empty string.
   */
  constructor({ baseUrl, customHeaders, key, cert } = {}) {
    this.httpClient = new HttpClient({ baseUrl, customHeaders, key, cert });
   ...
   *   request
   *
   * @return {Promise<Object>} FHIR resource or HTTP request headers, when no resource was
   */
   ...
   * console.log(response);
   *
   * // See CREATE operation documentation for a description of additional response metadata.
   *
   * @param {Object} params - The request parameters.
