...
class Client {
  ...
  /**
   * Search across all FHIR resource types.
   * Only the parameters defined for all resources can be used.
   *
   * @example
   *
   * // Using promises
   * fhirClient.search-all({ searchParams: { name: 'smith' } })
   *           .then((data) => { console.log(data); });
   *
   * // Using async
   * let response = await fhirClient.search-all({ searchParams: { name: 'smith' } });
   * console.log(response);
   *
   * @param {Object} params - The request parameters.
   * @param {Object} params.searchParams - The search parameters.
   ...
   */
  searchAll(params) {
    return this.httpClient.get(`?${new URLSearchParams(params.searchParams)}`);
  }
   ...
   *
   * @param {Object} params - The request parameters.
   * @param {string} params.resource - The new resource data to create.
   *
   * @return {Promise<Object>} FHIR resource
   */
  create(params) {
    return this.httpClient.post(params.resourceType, params.resource);
  }
   ...
   * @example
   *
   * const updatedPatient = { resourceType: 'Patient',
   *                          birthDate: '1948-04-14' }
   * // Using promises
   * fhirClient.update({ resourceType: 'Patient',
   *                     id: 12345,
   ...
   *
   * // Using async
   * let response = await fhirClient.update({ resourceType: 'Patient',
   *                                          id: 12345,
   *                                          resource: updatedPatient,
   *                                        });
   * console.log(response);
   ...
   * @param {string} params.resourceType - The resource type (e.g. "Patient", "Observation").
   * @param {string} params.id - The FHIR id for the resource.
   * @param {string} params.resource - The resource to be updated.
   *
   * @return {Promise<Object>} FHIR resource
   */
  update(params) {
    return this.httpClient.put(`${params.resourceType}/${params.id}`, params.resource);
  }
   ...
   */
  history(params) {
    return this.httpClient.get(`${params.resourceType}/${params.id}/_history`);
  }
   ...
   *
   * // Using promises
   * fhirClient.history-type({ resourceType: 'Patient' });
   *           .then((data) => { console.log(data); });
   *
   * // Using async
   * let response = await fhirClient.history-type({ resourceType: 'Patient' });
   * console.log(response);
   *
   * @param {Object} params - The request parameters.
   * @param {string} params.resourceType - The resource type (e.g. "Patient", "Observation").
   ...
   */
  history-type(params) {
    return this.httpClient.get(`${params.resourceType}/_history`);
  }
   ...
   *
   * // Using promises
   * fhirClient.history-all();
   *           .then((data) => { console.log(data); });
   *
   * // Using async
   * let response = await fhirClient.history-all();
   * console.log(response);
   *
   * @return {Promise<Object>} FHIR resources in a FHIR Bundle structure.
   */
   ...
   */
  batch(params) {
    // params.content is a Bundle with Bundle.type = batch
    return this.httpClient.post('/', params.content);
  }
   ...
   */
  transaction(params) {
    // params.content is a Bundle with Bundle.type = transaction
    return this.httpClient.post('/', params.content);
  }
}
