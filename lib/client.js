...
class Client {
   ...
   */
  read(params) {
    const url = this.buildUrl(`${params.resourceType}/${params.id}`);
    return httpGet(url);
  }
  ...
  *  @param {string} params.resourceType - The resource type (e.g. "Patient", "Observation").
   ...
   */
  vread(params) {
    const url = this.buildUrl(`${params.resourceType}/${params.id}/_history/${params.version}`);
    return httpGet(url);
  }
