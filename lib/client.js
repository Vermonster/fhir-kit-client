...
class Client {
  ...
  async smartAuthMetadata({ headers, options = {} } = {}) {
    return new Promise((resolve, reject) => {
      ...
      const resolving = [ false, false ];

      const options1 = options;
      options1.signal = controller1.signal;

      this.capabilityStatement({ options: deprecateHeaders(options1, headers)})
        .then(r => {
          resolving[0] = true;
          if (!resolving[1]) {
            console.info('aborting wellknown');
            controller2.abort();
          }
          return resolve(authFromCapability(r));
        })
        .catch(e => console.info('metadata was aborted'));

      const wellKnownUrl = this.baseUrl.replace(/\/*$/, "/") + ".well-known/smart-configuration";

      const options2 = options;
      options2.signal = controller2.signal;

      this.httpClient.request('GET', wellKnownUrl, options2)
        .then(r => {
          resolving[1] = true;
          if (!resolving[0]) {
            console.info('aborting metadata');
            controller1.abort();
          }
          return resolve(authFromWellKnown(r));
        })
        .catch(e => console.log(e) && console.info('wellknown was aborted'));
    });
  }
