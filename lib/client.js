  ...
  async smartAuthMetadata({ headers, options = {} } = {}) {
    ...
    class FetchQueue {
      ...
      safeAbortOthers(currentJob) {
        for (let a = 0, l = this.numJobs; a < l; ++a) {
          if (currentJob === this.jobs[a]) { continue; }
    ...
    return new Promise((resolve, reject) => {
      ...
      const wellKnownSmartUrl = `${this.baseUrl.replace(/\/*$/, '/')}.well-known/smart-configuration`;
      this.httpClient.request('GET', wellKnownSmartUrl, wellknownSmartJob.addSignalOption(options))
        .then((r) => {
      ...
      this.capabilityStatement(metadataJob.addSignalOption({ options: deprecateHeaders(options, headers) }))
        .then((r) => {
      ...
      const wellKnownOidcUrl = `${this.baseUrl.replace(/\/*$/, '/')}.well-known/openid-configuration`;
      this.httpClient.request('GET', wellKnownOidcUrl, wellknownOidcJob.addSignalOption(options))
        .then((r) => {
