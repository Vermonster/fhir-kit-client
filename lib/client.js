const { URLSearchParams } = require('universal-url');
...
class Client {
   ...
   */
  capabilityStatement({ headers } = {}) {
    if (!this.metadata) {
      this.metadata = this.httpClient.get('metadata', headers);
    }
  ...
  systemSearch({ searchParams, headers } = {}) {
    return this.httpClient.get(`/_search?${new URLSearchParams(searchParams)}`, headers);
  }
