...
function agentGenerator(url, agentOptions = {}) {
  ...
  // This should be (a copy of) agentOptions, but
  // if there is not already defined property 'keepAlive',
  // define it as true.
  const _agentOptions = agentOptions;
  if (!(_agentOptions.keepAlive === false || _agentOptions.keepalive === false)) {
    _agentOptions.keepAlive = true;
  }

  // would there be a case in which agent options change
  // based on request method (GET, PUT) rather than URL?
  if (url.startsWith('https')) {
    agentCache[url] = { agent: new https.Agent(_agentOptions) };
  } else {
    agentCache[url] = { agent: new http.Agent(_agentOptions) };
  }
...
module.exports = class {
  ...
  async request(httpVerb, url, options = {}, body) {
    ...
    // Start with baseRequestOptions, then add specific ones:
    // - headers
    // - method
    // - body
    // - all additional options
    // - agent (if available)
    // linting error for hasOwnProperty explanation: https://eslint.org/docs/rules/no-prototype-builtins
    const requestOptions = this.baseRequestOptions;
    // let keepalive = {};
    // if (!agentIsAvailable) {
    //   keepalive = {
    //     keepalive: options.hasOwnProperty('keepalive') ? options.keepalive : true,
    //   };
    // }
