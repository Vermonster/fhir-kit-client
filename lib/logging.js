...
const errorLogger = require('debug')('node-fhir-client-error');
const infoLogger = require('debug')('node-fhir-client-info');
...
function logRequestInfo(action, url, axios) {
  ...
  infoLogger(`${action.toUpperCase()} ${url.toString()}`);
...
module.exports = { logRequestError, logRequestInfo, logResponseInfo, logError };
