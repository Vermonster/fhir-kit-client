...
module.exports = class {
   ...
   */
  async resolve({ reference, context, headers } = {}) {
    if (context === undefined) {
      if (reference.startsWith('http')) {
        return this.resolveAbsoluteReference(reference, headers);
      }
      return this.client.httpClient.get(reference, headers);
    }
    if (reference.startsWith('#')) {
      return this.resolveContainedReference(reference, context, headers);
    }
    return this.resolveBundleReference(reference, context, headers);
  }
   ...
   * @return {Promise<Object>} FHIR resource
   */
  async resolveAbsoluteReference(reference, headers) {
    ...
    return (new Client({ baseUrl })).read({ resourceType, id, headers });
  }
   ...
   * @return {Promise<Object>} - FHIR resource
   */
  async resolveBundleReference(reference, bundle, headers) {
    ...
    if (!entry) {
      return this.resolve({ reference, headers });
    }
