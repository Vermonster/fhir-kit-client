...
describe('Pagination', () => {
  beforeEach(function () {
    this.pagination = new Pagination(mockHttpClient);
  });
  ...
  describe('#nextPage', () => {
    it('returns httpClient get for the next link', function () {
      ...
      expect(this.pagination.nextPage(results)).to.equal(url);
    });

    it('returns undefined if no next page exists', function () {
      const results = readFixture('search-results.json');
      expect(this.pagination.nextPage(results)).to.equal(undefined);
    });
  });
  ...
  describe('#prevPage', () => {
    it('returns httpClient get for the previous link', function () {
      ...
      expect(this.pagination.prevPage(results)).to.equal(url);
    });

    it('returns undefined if no previous page exists', function () {
      const results = readFixture('search-results.json');
      expect(this.pagination.prevPage(results)).to.equal(undefined);
    });
